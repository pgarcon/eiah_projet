# include <sys/types.h>
# include <unistd.h>
# include <stdlib.h>
# include <stdio.h>
#include <stdio.h>
#include <time.h>

extern void traitFils1(chaine);
extern void traitFils2(nb);

int main(char * chaine, int nb)
{
pid_t  idProc ;
int  rapport, numSignal, statut;

/* processus pere */
time_t t;

t=time(NULL);

printf("%s\n",ctime(&t));
/* creation du fils1 */
idProc = fork() ;
switch  (idProc )
    {
    case -1 : perror("echec fork");exit(1);   /* erreur */
    
    case 0 :  traitFils1(chaine); exit(1) ;   /* appel du traitement du fils1 */
    }
    
/* creation du fils2 */    
idProc = fork() ;
switch  (idProc )
    {
    case -1 : perror("echec fork");exit(1);   /* erreur */
    
    case 0 :  traitFils2(nb); exit(1) ;   /* appel du traitement du fils2 */
    }

    
/* suite du processus pere */
/* attente de la terminaison des fils */

idProc = wait( &rapport ) ;
while  ( idProc != -1 )
    {    
    printf("\nTerminaison du fils de PID= %d\n", idProc);
    
    numSignal = rapport & 0x7F ;    
    switch ( numSignal )
    {
    case 0 :    /* fin normale */
            statut = (rapport >>8)& 0xFF  ;
            printf("Fin normale, statut= %d\n", statut); break ;
    
    default :    /* fin anormale */
            printf("Fin anormale, numSignal= %d\n", numSignal);
    
    }
    idProc = wait( &rapport ) ;
    }
}
/* fin du processus pere */
